...
#define MY_UUID { 0x1D, 0x6C, 0x7F, 0x01, 0xD9, 0x48, 0x42, 0xA6, 0xAA, 0x4E, 0xB2, 0x08, 0x42, 0x10, 0xEB, 0xBC }
PBL_APP_INFO(MY_UUID,
             "Menu Layer Demo", "Demo Corp",
             1, 0, /* App version */
             DEFAULT_MENU_ICON,
    ...
    case 0:
      ...
      switch (cell_index->row) {
        ...
        case 1:
          ...
          menu_cell_basic_draw(ctx, cell_layer, "Icon Item", "Select to cycle", &menu_icons[current_icon].bmp);
          break;

        case 2:
          // Here we use the graphics context to draw something different
          // In this case, we show a strip of a watchface's background
          graphics_draw_bitmap_in_rect(ctx, &menu_background.bmp,
              (GRect){ .origin = GPointZero, .size = cell_layer->frame.size });
          break;
      }
      break;
...
void window_load(Window *me) {
  ...
  heap_bitmap_init(&menu_icons[num_menu_icons++], RESOURCE_ID_IMAGE_MENU_ICON_BIG_WATCH);
  heap_bitmap_init(&menu_icons[num_menu_icons++], RESOURCE_ID_IMAGE_MENU_ICON_SECTOR_WATCH);
  heap_bitmap_init(&menu_icons[num_menu_icons++], RESOURCE_ID_IMAGE_MENU_ICON_BINARY_WATCH);

  // And also load the background
  heap_bitmap_init(&menu_background, RESOURCE_ID_IMAGE_BACKGROUND_BRAINS);
